...
//        接收客户端发送的审核结果
        String receive = ObtainData.obtain_data(request);
        mapReceive = gson.fromJson(receive, new TypeToken<Map<String, String>>() {
        }.getType());
        String user_id = mapReceive.get("user_id");


//        将审核结果存入数据库并返回结果给客户端
        boolean check = operateDB.update_user_review(user_id);
//        将用户信息存入用户表
